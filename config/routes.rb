Rails.application.routes.draw do
  ...
  GET /users/new # show me the new user signup form POST /users # the form
                 # action for signup, creates a user

  GET /login # show the login page POST /login # set the user
            #  id in the session if password checks out DELETE
            # /login # remove user id from session

  GET /links/new # show the form for a new link POST /links # the f
                 # orm action for new link, creates a link (if logged
                 #  in) GET /link/:id # takes you to the thing it links to

  GET /link/:id/comments # get the page for a link, its comments, and comment
                         # form POST /link/:id/comments # the form action for new comment,
                         #  creates a comment (if logged in)

  GET /link/:id/comments # get the page for a link, its comments, and
